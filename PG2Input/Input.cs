...
namespace PG2Input
{
    public static class Input
    {
        ...
        public static int ReadInteger(string prompt, int min, int max)
        {
            ...
            while(!int.TryParse(msg, out num) || num > max || num < min)
            {
                Console.WriteLine("Make sure the number is in the integer range and is a valid integer. Please try again.");
        ...
        public static void ReadString(string prompt, ref string value)
        {
            ...
            {
                Console.WriteLine("Please enter a value and don't just leave it empty!!!!!");
                Console.Write($"{prompt}: ");
                value = Console.ReadLine();
            }
        ...
        public static void ShowHistogram(string word, int count)
        {
            Console.CursorLeft = 5;
            Console.Write($"{word,15}  ", Console.CursorLeft) ;
        ...
        public static void FindSentences(string search, Dictionary<string,int> list)
        {
            string[] sentence = Input.GetSpeech().Split(new char[] { '.', '!', '?' }, StringSplitOptions.RemoveEmptyEntries);

            for (int i = 0; i < sentence.Length; i++)
            {
                if(sentence[i].ToLower().IndexOf($"{search}") > -1)
